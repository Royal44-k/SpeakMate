...
后续为G专项复用，将本文件自有3132服务的beforeAll／afterAll提取到 `tests/e2e/task7-owned-local-server.ts`。B／C／E业务步骤、等待条件和断言不变；新的宿主生命周期由G两项真实用例12.2秒通过覆盖，三份QA文件ESLint与类型检查通过。未把此前B／C／E结果伪称提取后又完整运行了一次。
