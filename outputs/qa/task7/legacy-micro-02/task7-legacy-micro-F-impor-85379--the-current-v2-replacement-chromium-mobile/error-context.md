...
- generic [active] [ref=f2e1]:
  - alert [ref=f2e2]
  - main [ref=f2e3]:
    - paragraph [ref=f2e7]: PRACTICE SAVED
    - heading "这轮已保存。" [level=1] [ref=f2e8]
    - paragraph [ref=f2e9]: 所选流程的目标已由本地收录表达确认。 这不是语言能力评分，也不代表真实服务已完成。
    - link "查看本次复盘" [ref=f2e10] [cursor=pointer]:
      - /url: /session/report?id=simulation_e840c9fc-6cc2-487d-8b7e-cee51d2916c2
    - link "返回来源页面" [ref=f2e11] [cursor=pointer]:
      - /url: /notebook/note?id=legacy-micro-note
    - link "返回词句或记录簿" [ref=f2e12] [cursor=pointer]:
      - /url: /notebook/note?id=legacy-micro-note
  - status [ref=f2e13]
```
...
```ts
  ...
  20 |   const note={...base.notebook[0],id:'legacy-micro-note',profileId:base.profile.id,text:'on the side',normalizedText:'on the side',kind:'phrase' as const,sources:[source],favoriteIds:[],notes:'Historical v1 pinned fixture',tags:[]}
