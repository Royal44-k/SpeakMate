...
Locator: getByRole('button', { name: '返回顶部', exact: true })
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: '返回顶部', exact: true })
...
```ts
  ...
  2   | import { createServer, request as httpRequest, type Server } from 'node:http'
  3   | import { readFile, writeFile } from 'node:fs/promises'
  4   | import { createHash } from 'node:crypto'
  5   | import vm from 'node:vm'
  6   | import { onboard, enterScene, capture, localState, protect } from './task7-browser-helpers'
  7   | 
  8   | // QA-only transparent same-origin transport. Both upstreams are actual frozen
  9   | // Next builds with their matching public directory; no route/body substitution.
  10  | let upstream = 3133
  11  | let server: Server
  12  | const transport: unknown[] = []
  13  | test.use({ baseURL: 'http://127.0.0.1:3134' })
  14  | test.beforeAll(async () => {
  15  |   server = createServer((req, res) => {
  16  |     const selected = upstream
  17  |     const forward = httpRequest({ hostname: '127.0.0.1', port: selected, path: req.url, method: req.method, headers: req.headers }, response => {
  18  |       transport.push({ upstream: selected, path: req.url, method: req.method, status: response.statusCode })
  19  |       res.writeHead(response.statusCode!, response.headers)
  20  |       response.pipe(res)
  21  |     })
  22  |     forward.on('error', error => res.destroy(error))
  23  |     req.pipe(forward)
  24  |   })
  25  |   await new Promise<void>((resolve, reject) => { server.once('error', reject); server.listen(3134, '127.0.0.1', resolve) })
  26  | })
...
> 98  |   await expect(page.getByRole('button', {name:'返回顶部', exact:true})).toBeVisible()
  ...
  121 |   const simulationId = new URL(editing.url()).searchParams.get('id')!
  122 |   await editing.close()
  123 |   // Closing the actual simulation legitimately flushes its foreground segment;
  124 |   // wait for that owned transaction before taking the update checkpoint.
  125 |   await expect.poll(async () => (await localState(page)).learningEvents.some(event => event.type === 'foreground-time-recorded' && event.runId === simulationId)).toBe(true)
  126 |   const committedBeforeUpdate = await localState(page)
  127 |   await page.goto('/me')
  128 |   const reload = page.waitForEvent('load')
  129 |   const newcomer = await context.newPage()
  130 |   // Race a real newly arriving in-scope document against explicit activation.
  131 |   const arriving = newcomer.goto('/notebook')
  132 |   await page.getByRole('button', { name: '立即更新', exact: true }).click()
  133 |   await arriving
  134 |   const racedBuild = await buildId(page)
  135 |   if (racedBuild === old.buildId) {
  136 |     await expect(page.getByText(/请先完成并关闭其他 SpeakMate 窗口，再重试更新；/)).toBeVisible()
  137 |     await newcomer.close()
  138 |     await page.getByRole('button', {name:'立即更新',exact:true}).click()
  139 |   } else {
  140 |     expect(racedBuild).toBe(latest)
  141 |     await expect(newcomer.getByRole('heading',{name:'记录簿',exact:true})).toBeVisible()
  142 |     await newcomer.close()
  143 |   }
  144 |   await reload
  145 |   await expect(page.getByRole('region', { name: '本机个人成绩' })).toBeVisible()
  146 |   expect(await buildId(page)).toBe(latest)
  147 |   const after = await localState(page)
  148 |   expect(after).toEqual(committedBeforeUpdate)
  149 |   const cachesKept = await page.evaluate(() => caches.keys())
  150 |   expect(cachesKept).toContain(`speakmate-build-v1-${old.buildId}`)
  151 |   expect(cachesKept).toContain(`speakmate-build-v1-${latest}`)
  152 |   await writeFile(info.outputPath('real-update.json'), JSON.stringify({ old: old.buildId, latest, lazy, layout, combined, racedBuild, cachesKept, after, transport, oldManifestHash: createHash('sha256').update(oldManifestSource).digest('hex') }, null, 2))
  153 |   await finishNetwork(info)
  154 | })
  155 | 
```