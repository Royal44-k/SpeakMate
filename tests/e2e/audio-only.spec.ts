...
test('recorded audio can be submitted without browser speech recognition', async ({ page, browserName }) => {
  test.skip(browserName !== 'chromium', 'MediaRecorder mock targets the Chromium recording path.')
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, 'AudioContext', {
      value: class { constructor() { throw new Error('meter unavailable') } },
    })
  ...
  await page.route('**/api/v1/turns', async (route) => {
    ...
    receivedAudioOnly = body.includes('name="audio"') && !body.includes('name="transcript"')
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        reply: { text: 'May I see your passport?', hintZh: '继续确认身份。', emotion: 'warm' },
        ...
        provider: 'cloudflare', degraded: false, requestId: 'req_audio', latencyMs: 20,
      }),
    })
  })
  ...
  await page.getByRole('button', { name: '停止并检查' }).click()
