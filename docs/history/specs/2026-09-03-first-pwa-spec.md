...
移动键盘弹出时隐藏摄影横幅，保证输入框和提交按钮不被遮挡。使用 `100dvh`、`env(safe-area-inset-bottom)` 和可滚动内容区处理 iPhone 浏览器工具栏及安全区域。
...
- Android/Chromium 可监听 `beforeinstallprompt`；iOS 不依赖该事件，显示“Safari 分享 → 添加到主屏幕”的分步说明。
...
- 视口：360 × 800、390 × 844、393 × 852、430 × 932、768 × 1024。
