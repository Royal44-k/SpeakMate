...
> 2026-09-13 归档导航：本页全文保留 2.x 历史操作，请使用新的 [3.0 部署与应急手册](engineering/DEPLOYMENT_3.md)。下文日期与状态只代表记录当时，不覆盖当前候选部署与用户可访问性反馈。

> 版本边界（2026-09-10）：下文保留的是 **2.x 历史操作与发布记录**，不是正在开发的 3.0 操作指南。3.0 严格零新增费用、设备本地学习，不允许按旧第 4 节开启云 AI 或同步，也不新建项目或服务。现有生产仍为 2.3，3.0 未验收、未发布；请先看[3.0 验证记录](releases/3.0.0-verification.md)、[当前规格](../APP_SPEC.md)与[iPhone 安装自查](guides/3.0-iphone-setup-and-checks.md)。发布前必须验证可读取 IndexedDB v2 且保留新数据的兼容回退构建，不能直接把下文的旧部署当作升级设备的数据安全回退。历史网络、设备和版本结论只适用于记录对应的时间与构建。
