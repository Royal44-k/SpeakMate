...
- [ ] 生成逐文件 SHA256 清单、源码 ZIP、完整 Git bundle 和交付校验记录。
- [ ] 实际读取 ZIP、校验哈希、验证 bundle/提交历史及恢复说明。

## Task 4：上传门槛与远端核验

- [x] 找到并核对本机已有 Royal44-k 登录态；三分支原子推送 dry-run 成功，未读取秘密，不必更换连接器。
- [ ] 写入前重查仓库，非空则比较并保留现有历史，不强制覆盖。
- [ ] 明确推送 main 和两条归档分支，读取远端 SHA/tree 验证。
- [ ] 填写真实上传结果与提交链接。若权限仍缺失，交付本地包并明确报告阻挡。
