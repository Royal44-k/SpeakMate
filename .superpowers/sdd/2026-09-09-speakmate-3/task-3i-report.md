...
## Fix round 1 of 5 — DONE_WITH_CONCERNS / scoped re-review pending (2026-09-11)

Independent review of `ed5481c5760ce005b18ab7d0eedd646fe45ffe09` found I1–I4 (4 Important); this section supersedes the prior implementation completion status until scoped re-review. Controller-only HEAD at fix start `5c4149d` records Ruling21: unusual accepted stored IDs remain unchanged and get in-place readonly access/export, not wider new routes or tighter backups. No controller files are edited. Same portable Node22 and Vitest executable below; no helpers/browser/deployment/new dependency. Earlier production build/hash evidence is historical after these runtime fixes, not proof of the changed build.

- I1 RED: `node_modules/vitest/vitest.mjs run src/features/practice/practice-stage-integration.test.tsx`,11:30:11,17.49s,1failed: real stage initial render throws `无效的本地学习目标` at eager report link, before real hook create effect.
- I1 GREEN: `node_modules/vitest/vitest.mjs run src/features/practice/practice-stage-integration.test.tsx src/features/practice/practice-stage.test.tsx src/features/practice/use-practice-session.test.tsx src/features/practice/session-resolver.test.tsx`,11:31:13,22.08s,4files29passed. Minimal stage guard before record-only links; hooks still unconditional and reserved report/new remains invalid. New test uses real stage+hook+memory repository in StrictMode, asserts initial loading, exactly one stored pinned session, real-ID address replacement, actual first question and zero input turns. Only absent host Next router is substituted, not practice state.
- I2 RED: `node_modules/vitest/vitest.mjs run src/components/app-shell/learning-routes.test.ts src/infrastructure/persistence/offline-worker.test.ts 'src/app/scenes/[slug]/page.test.tsx'`,11:32:47,2.64s,17failed/34passed. Eight online conversions silently omitted empty values; eight online-vs-SW parity checks failed on the online side (SW already rejected); real legacy page forwarded empty level. Same command11:33:16,2.80s,3files51passed. Builder now serializes present-empty public options so existing strict parser rejects them; absent defaults remain valid. No worker runtime/cache change.
- I3 RED: `node_modules/vitest/vitest.mjs run src/features/practice/practice-home.test.tsx src/features/profile/learning-center.test.tsx src/infrastructure/persistence/graded-backup.test.ts`,11:35:20,23.91s,4failed/10passed plus4uncaught render errors from the reported throwing link. Actual v1 golden fixture restored with121ASCII/nonASCII IDs and unchanged valid matching turn/favorite references. Both backup adapters already round-tripped these IDs (direct regression validation, not a new RED claim). First correction run11:37:26,22.01s,4failed/10passed: recovered UI now rendered but text selector matched both learner sentence and identical saved correction. Narrowed assertion to actual English transcript paragraph, retaining feedback assertion. Same command11:39:24,22.88s,3files14passed. `savedPracticeHref(id,kind)` returns no link for unsupported/reserved stored IDs without changing input bounds; both entry surfaces show explicit in-place readonly transcript using existing coherent record/HistoricalPracticeRecord. Export points to existing privacy/data controls; fixture exports prove all IDs/rows/references preserved. No alias/new store/backup schema rewrite.
- I4 hook RED: `node_modules/vitest/vitest.mjs run src/features/practice/use-practice-session.test.tsx -t 'delayed recovery read'`,11:36:07,3.59s,3failed/12filtered. Deferred read restores A over edited B, resurrects A after cancellation, and restores stale state after successful submit. Real-stage RED: `node_modules/vitest/vitest.mjs run src/features/practice/practice-stage-integration.test.tsx -t 'newer visible draft'`,11:38:23,17.50s,1failed/1filtered; actual textbox B becomes A.
- I4 owns its read via a local generation+expected-record identity and applies the latest draft through a functional React state update. Submit/stop/finish, cancel, recording/suggestion/change and exit invalidate older reads; unmount invalidates too. Visible reading status explicitly permits editing, with cancel/submit causing late results to be ignored. First covering run11:40:41,14.41s,32passed/1failed: hook races GREEN but new visible-status assertion caught disabled flag on wrong button. Corrected the specific recovery button; no broader machine/global state framework.
- I4 final covering GREEN: same four-file I1 covering command,11:41:59,13.03s,4files33passed. Includes three deferred real-hook races and real-stage A→B→read→submit B, plus existing recording/settings/pinned retry/resolver/terminal coverage. No hook-ready mock in either new composition regression. Filtered cases in RED commands were `-t` exclusions, not disabled tests.
- Additional I2/I3 consumer validation: `node_modules/vitest/vitest.mjs run src/features/practice/legacy-learning-shell.test.tsx src/components/app-shell/learning-routes.test.ts src/infrastructure/persistence/offline-worker.test.ts 'src/app/scenes/[slug]/page.test.tsx'`,11:44:12,18.78s,4files55passed. Real legacy/new static shells with real default local repositories leave zero sessions/turns for empty level/mode/round and SW-invalid redirect form; ordinary saved IDs retain their exact links and reserved/unsupported IDs never produce links. These added checks passed against the already corrected boundary, not retroactive RED. Combined nonoverlapping covering files:11files102tests passed (55 routing/shell/worker +33 practice +14 history/backup); not a new complete6,155-test-suite run.

Fix self-review: all four findings checked against the final changed runtime and test composition; no current corpus prose, engine, snapshot schema, data stores, routing bounds, worker cache protocol or generated asset changes. Loading guard remains after unconditional hooks and before saved-record links; default/empty distinction flows through the existing parser; retained records display full original learner/assistant transcript and historical feedback with explicit readonly/no-route/no-resume explanation and real existing `/privacy` export entry; backup fixtures exercise existing v1 restore and v2 export/restore through both adapters. New PracticeRecord-only selection state is local to each existing entry page, not an alias/router/store. `savedPracticeHref(id:string,kind:'session'|'report'): string|undefined` is the exact new stored-link handoff; new launch builders are unchanged. React review kept functional draft updates and generation ownership local; canceled/submitted generations cannot apply late read success or failure. Small existing-style button rules preserve tap-target intent, palette and fonts; actual layout/focus/device acceptance remains later. Formatting accidentally expanded the compact learning-center CSS; restored its existing formatting and retained only three new conditional-entry rules.

Final real-stage check after self-review added a pass-through spy on the REAL guarded commit: `node_modules/vitest/vitest.mjs run src/features/practice/practice-stage-integration.test.tsx`,11:48:16,18.30s,1file2passed. In addition to one durable row it now explicitly verifies exactly one real create call in StrictMode, so duplicate same-ID creation attempts cannot hide behind idempotent storage. This is strengthening the genuine I1 regression, not a new RED claim.

Type/lint: portable `node_modules/typescript/bin/tsc --noEmit` started11:44:50; changed-file ESLint started11:45:03; both completed11:46:40 with no diagnostics. Exact ESLint invocation is listed below. These initial command wrappers appended a timestamp. Following the final test-only assertion, direct `node_modules/typescript/bin/tsc --noEmit` returned exit0/no output in4.81s, and direct `node_modules/eslint/bin/eslint.js src/features/practice/practice-stage-integration.test.tsx` returned exit0/no output in3.18s, both completed before11:49:19. No full suite or production build was repeated. `git diff --check`11:49:19 exit0, only repository LF→CRLF notices.

No source mismatch or architecture decision is outstanding. Initial diagnostic processes were slower than focused tests; only their exact owned sessions were polled, no duplicate process/server launched or unrelated process killed. A read-only Get-Process check subsequently reported the two completed process IDs absent; not an application error. Prior production hash/bytes remain evidence of the pre-fix artifact only. Controller explicitly deferred a new full suite/build to later gates; no browser/SW/device/presentation acceptance is claimed here. Independent scoped re-review of I1–I4 is required before the gate becomes approved.

### Fix-round command and commit scope

All command fragments in this round are prefixed by `& .superpowers/sdd/2026-09-09-speakmate-3/runtimes/node-v22.23.2-win-x64/node.exe`, cwd `D:/Codex-chat/SpeakMate/.worktrees/speakmate-3`. Changed-file lint arguments:

```text
node_modules/eslint/bin/eslint.js src/app/scenes/[slug]/page.test.tsx src/components/app-shell/learning-routes.test.ts src/components/app-shell/learning-routes.ts src/features/practice/historical-practice-record.tsx src/features/practice/practice-home.test.tsx src/features/practice/practice-home.tsx src/features/practice/practice-stage.test.tsx src/features/practice/practice-stage.tsx src/features/practice/use-practice-session.test.tsx src/features/practice/use-practice-session.ts src/features/profile/learning-center.test.tsx src/features/profile/learning-center.tsx src/infrastructure/persistence/graded-backup.test.ts src/infrastructure/persistence/offline-worker.test.ts src/features/practice/legacy-learning-shell.test.tsx src/features/practice/practice-stage-integration.test.tsx tests/fixtures/historical-id.ts
```

Fix implementation changes the following19files only, plus this report in an evidence-only follow-up. Pre-fix source reviewed at `ed5481c5760ce005b18ab7d0eedd646fe45ffe09`; controller documentation preserved through pre-fix-commit HEAD `f0c17383ad403a45995f9989297d5d70b456da38`. Original BASE remains unchanged. No controller brief/review/contract/ledger staged by implementer.

Fix commit `288fdf772ce4005848c9cdd0b63ea305d5cfdd8c` (`fix: close graded practice initialization and recovery review gaps`), parent exactly `f0c17383ad403a45995f9989297d5d70b456da38`,19files/640insertions/86deletions. Before commit, explicit staged names matched the19file allowlist and `git diff --cached --check` exited0. At11:50:42 postcommit status contained only this report; no source changes remained. This report-only follow-up supplies the durable fix evidence and commit metadata; its own SHA is given in the final handoff. Scoped re-review range begins at prior reviewed `ed5481c5760ce005b18ab7d0eedd646fe45ffe09`, preserving intervening controller-only documentation commits.

```text
src/app/practice/practice-home.module.css
...
src/features/profile/learning-center.module.css
...
src/features/practice/legacy-learning-shell.test.tsx
src/features/practice/practice-stage-integration.test.tsx
tests/fixtures/historical-id.ts
```

The original implementation narrative below is historical. Its full-suite/build claims apply to its recorded pre-review source and artifacts, not the fix-round source. Fix-round results and remaining concerns above supersede the prior integration handoff.

Task-owned original implementation and self-review were completed with full unit suite, typecheck, changed-file lint, production build and direct HTTP artifact checks. Independent review subsequently found I1–I4 as recorded above. This is not a release or browser-offline acceptance claim. Parent/controller documentation commits are preserved. Earlier milestones below remain chronological, including genuine failures and superseded interim artifacts.
